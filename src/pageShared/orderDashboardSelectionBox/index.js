        ...
        <View style={{ flexDirection: 'row', flexWrap: 'wrap' }}>
            {data.map((item, key) => (
                <TouchableOpacity style={{ marginTop: 15 }} key={key} activeOpacity={0.9} onPress={() => onSelect(item)}>
                    <View style={{ backgroundColor: item.backgroundColor ? item.backgroundColor : backgroundColor, justifyContent: 'center', alignItems: 'center', borderRadius: 15, height: 120, width: Dimension.width / 3.7, marginHorizontal: 5 }}>
                        {item.redIcon == true ?
