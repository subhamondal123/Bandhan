...
import { SafeAreaView, Text, View, Image, FlatList, ActivityIndicator, RefreshControl, Platform, KeyboardAvoidingView } from 'react-native';
...
class GamificationReferLead extends React.Component {
    ...
    _load = async () => {
        ...
        this.setState({ countryArr: modifyCountryArrData(countryArr) })
        this.state.countryObj.id = userCredential.locationData[0].countryId;
        this.setState({ countryObj: this.state.countryObj });
        ...
        // }
        await this.setStateData(userCredential.locationData[0].countryId, userCredential.locationData[0].stateId);
        await this.setDistrictData(userCredential.locationData[0].stateId, userCredential.locationData[0].districtId);
        await this.setZoneData(userCredential.locationData[0].districtId, userCredential.locationData[0].zoneId);
    }
    ...
    setStateData = async (countryId, stateId) => {
        ...
        if (responseData) {
            if (responseData.status === ErrorCode.ERROR.ERROR_CODE.SUCCESS) {
                ...
                // this.state.stateObj.id = this.state.stateArr.id;
                // this.state.stateObj.name = this.state.stateArr.name;
    ...
    getLandingData = async () => {
        ...
        let responseData = await MiddlewareCheck("enqueryMasterDataDropdown", {}, this.props);
        if (responseData === false) {
        } else {
            if (responseData.status === ErrorCode.ERROR.ERROR_CODE.SUCCESS) {
    ...
    getBrandData = async () => {
        ...
        if (responseData == false) {
            ...
            if (responseData.status === ErrorCode.ERROR.ERROR_CODE.SUCCESS) {
                modifiedData = await modifyConversionLanding(responseData.response);
    ...
    checkMobileNoExistOrNot = async (txt) => {
        ...
        let responseData = await MiddlewareCheck("customerPhoneExist", reqData, this.props);
        if (responseData) {
            if (responseData.status === ErrorCode.ERROR.ERROR_CODE.SUCCESS) {
                Toaster.ShortCenterToaster(responseData.message)
    ...
    buttonSection = () => {
        ...
        const onSubmitLeadData = async () => {
            ...
            // if (validatedData.status) {
            //     this.setState({ addLoader: true })
            //     let responseData = await MiddlewareCheck("addInternalEnquiry", reqData, this.props);
            //     if (responseData === false) {
            //     } else {
            //         if (responseData.status === ErrorCode.ERROR.ERROR_CODE.SUCCESS) {
            //             Toaster.ShortCenterToaster(responseData.message);
            ...
            if (validatedData.status) {
                this.setState({ addLoader: true })
                let responseData = await MiddlewareCheck("addNewEnquiry", reqData, this.props);
                if (responseData === false) {
                } else {
                    if (responseData.status === ErrorCode.ERROR.ERROR_CODE.SUCCESS) {
                        Toaster.ShortCenterToaster(responseData.message);
